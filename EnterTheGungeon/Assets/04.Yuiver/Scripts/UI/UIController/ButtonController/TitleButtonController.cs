...
public class TitleButtonController : MonoBehaviour
{
    ...
    //Continiue 버튼의 활성화 여부를 결정하기 위해서 만들어둔 변수
    ...
    private void Start()
    {
        ...
        optionActive = false;
        hasValidSaveData = loadOptionData.hasValidSaveData;
        ...
        //컨티뉴 버튼을 켤지 껄지 결정합니다.
        buttonHandlers[1].gameObject.SetActive(hasValidSaveData);
    ...
    private void Update()
    {
        ...
        if (optionActive == false)
        {
            ...
            {
                if (!hasValidSaveData && buttonIndex == 2)
                {
                    ActivateSingleButton(LimitKeyBoardIndex(buttonIndex + (-2)));
                }
                else
                {
            ...
            {
                if (!hasValidSaveData && buttonIndex == 0)
                {
                    ActivateSingleButton(LimitKeyBoardIndex(buttonIndex + 2));
                }
                else
